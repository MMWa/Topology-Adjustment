...
class WindowManager:
    ...
    def __init__(self):
        ...
        self.surface = pygame.Surface(self.screen.get_size(), pygame.SRCALPHA, 32)
    ...
    def __draw(self, algorithm_delta):
        ...
        for x in self.pursue_relays:
            pygame.draw.circle(self.surface, self.GREEN, self.to_pygame(x, point_height), point_radius)

        for x in self.greedy_relays:
            pygame.draw.circle(self.surface, self.BLUE, self.to_pygame(x, point_height), point_radius)

        for x in self.nodes:
            pygame.draw.circle(self.surface, self.RED, self.to_pygame(x, point_height), point_radius)
