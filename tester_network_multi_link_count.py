...
if __name__ == "__main__":
        ...
        for _ in range(AVERAGE_SAMPLES):
            ...
            for idx, x in enumerate(combined_simulator[:4]):
                if idx == 2:
                    for _ in range(30):
                        x.execute_pipeline()
                    print_list.append(multi_link(x.relay_list, UNIT_DISTANCE, "cpD"))

                else:

                    x.execute_pipeline()
                    # print(len(x.relay_list))
                    # print_list.append(len(x.relay_list))
                    print_list.append(multi_link(x.relay_list + x.node_list, UNIT_DISTANCE))
                    x.reset()
        #
    ...
    plt.grid(True)
    plt.legend([ "Improved Pursue Algorithm", "Greedy Algorithm", "Decentralized Algorithm"])
    plt.xlabel("Number of Nodes")
    plt.ylabel("Number of Multi-Link Connections")
    plt.show()
    #
